...
// 传入 json 类型数据
// 向 stock_now_data 表中循环插入数据
func InsertNowData(json string, hy_code string) (bool, error) {
	// connect databases
	db := databases.MysqlINIT()
	defer db.Close()
	//结构体
	compant_data := CompanyData{}
	//c_data := []CompanyData{}
	// 处理json 数据
	...
	for i = 0; i < value5.Int(); i++ {
		...
		stmt, err := db.Prepare("insert into `stock_now_data`(stock_code,stock_wy_code,stock_hy_code,stock_name,stock_price,stock_open,stock_five_minute,stock_high,stock_hs," +
		...
		//使用Stmt对象执行预处理参数
		_, err = stmt.Exec(compant_data.CODE, compant_data.WY_CODE, compant_data.HY_CODE, compant_data.NAME, compant_data.PRICE, compant_data.OPEN, compant_data.FIVE_MINUTE, compant_data.HIGH, compant_data.HS,
